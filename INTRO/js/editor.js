document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderEditor() {
        ...
        transitionSection.className = 'editor-card';
        const t = gameConfig.transition || {};
        transitionSection.innerHTML = `
            <h2>2. 화면 전환(페이드) 설정</h2>
            <div class="input-group">
                <label>이미지 페이드 아웃 (ms):</label>
                <input type="number" min="0" value="${Number.isFinite(Number(t.imageFadeOutMs)) ? Number(t.imageFadeOutMs) : 800}"
                    onchange="updateTransitionConfig('imageFadeOutMs', parseInt(this.value, 10))">
            </div>
            <div class="input-group">
                <label>이미지 페이드 인 (ms):</label>
                <input type="number" min="0" value="${Number.isFinite(Number(t.imageFadeInMs)) ? Number(t.imageFadeInMs) : 800}"
                    onchange="updateTransitionConfig('imageFadeInMs', parseInt(this.value, 10))">
            </div>
            <div class="input-group">
                <label>음악 페이드 아웃 (ms):</label>
                <input type="number" min="0" value="${Number.isFinite(Number(t.audioFadeOutMs)) ? Number(t.audioFadeOutMs) : 800}"
                    onchange="updateTransitionConfig('audioFadeOutMs', parseInt(this.value, 10))">
            </div>
            <div class="input-group">
                <label>음악 페이드 인 (ms):</label>
                <input type="number" min="0" value="${Number.isFinite(Number(t.audioFadeInMs)) ? Number(t.audioFadeInMs) : 800}"
                    onchange="updateTransitionConfig('audioFadeInMs', parseInt(this.value, 10))">
            </div>
            <p style="margin-top:8px; color:#aaa; font-size:0.9rem;">
                값은 즉시 저장되며, 실제 인트로 재생 화면에서 적용됩니다.
            </p>
        `;
        editorApp.appendChild(transitionSection);

        // 씬 설정 섹션
        const scenesSection = document.createElement('div');
        scenesSection.innerHTML = '<h2>3. 화면별 상세 설정</h2>';
    ...
    window.updateTransitionConfig = (prop, val) => {
        if (!gameConfig.transition || typeof gameConfig.transition !== 'object') {
            gameConfig.transition = {};
        }
        const n = Number(val);
        const safe = Number.isNaN(n) ? 0 : Math.min(30000, Math.max(0, Math.floor(n)));
        gameConfig.transition[prop] = safe;
        saveAndRender();
    };
